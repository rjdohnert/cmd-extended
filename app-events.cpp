...
int main() {
    ...
    if (!IsRunAsAdmin()) {
        wprintf(L"This program requires administrator privileges to read application logs.\n");
    ...
    // Target the Application event log channel
    LPCWSTR channelPath = L"Application";
    LPCWSTR query = L"*";

    // Query Application log channel for newest events first
    EVT_HANDLE hResults = EvtQuery(NULL, channelPath, query, EvtQueryChannelPath | EvtQueryReverseDirection);
    if (hResults == NULL) {
        PrintError(L"Failed to query application event logs.");
        return 1;
    }

    wprintf(L"Parsing application events (newest first):\n\n");
    ...
    if (response == L'y' || response == L'Y') {
        wprintf(L"Enter filename/path (default: application_log.txt): ");
        ...
        if (fgetws(filepath, MAX_PATH, stdin) != NULL) {
            ...
            if (wcslen(filepath) == 0) {
                wcscpy_s(filepath, L"application_log.txt");
            }
        } else {
            wcscpy_s(filepath, L"application_log.txt");
        }
