...
int main() {
    ...
    // Target the Security event log channel
    LPCWSTR channelPath = L"Security";
    LPCWSTR query = L"*";

    // Query Security log channel for newest events first
    ...
    if (response == L'y' || response == L'Y') {
        wprintf(L"Enter filename/path (default: security_log.txt): ");
        ...
        if (fgetws(filepath, MAX_PATH, stdin) != NULL) {
            ...
            if (wcslen(filepath) == 0) {
                wcscpy_s(filepath, L"security_log.txt");
            }
        } else {
            wcscpy_s(filepath, L"security_log.txt");
        }
